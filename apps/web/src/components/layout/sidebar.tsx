...
/**
 * 顧客画面プレビューの遷移先 URL を現在のホストから算出する。
 * team ドメインでは同一オリジンの /client は DomainGuard に弾かれるため、
 * 顧客が実際に使うオリジン (app / staging) を別タブで開く。
 */
function clientPreviewUrl(): string {
  if (typeof window === 'undefined') return '/client'
  const host = window.location.hostname
  if (host === 'team.line-port.com') return 'https://app.line-port.com/client'
  if (host === 'staging-team.line-port.com') return 'https://staging.line-port.com/client'
  // localhost / pages.dev / staging-team 検証などは同一オリジンで表示可
  return '/client'
}
...
const menuSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/broadcasts/segments', label: 'セグメント配信', icon: 'M7 7h10M7 12h10M7 17h6M19 7l2 2-2 2M19 17l2-2-2-2' },
      { href: '/research', label: 'リサーチ', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
      ...
      { href: '/card-messages', label: 'カード型メッセージ', icon: 'M7 4h10c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H7c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2zm0 4h10M7 12h10M7 16h6' },
      { href: '/coupons', label: 'クーポン', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3M8 12h.01M8 16h.01' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/engagement', label: 'エンゲージメント計測', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
      ...
      { href: '/diagnosis-leads', label: '診断リード', icon: 'M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z' },
      { href: '/consultation-leads', label: '無料相談リード', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 3v-3z' },
      { href: '/cta-analytics', label: 'CTA計測', icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' },
  ...
  },
  {
    label: 'L-port AI',
    items: [
      { href: '/onboarding', label: 'オンボーディング', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
      { href: '/tenants', label: 'アカウント運用', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
      { href: '/agent', label: '自動化ダッシュボード', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
      { href: '/ai-prompts', label: 'AI 配信設定', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
      { href: '/ai-products', label: '商品データベース', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
      { href: '/chat-preview', label: 'AI 接客プレビュー', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
      { href: '/playbook-library', label: '実例ライブラリ', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
      { href: '/client', label: '顧客画面プレビュー', icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z' },
      { href: '/reports', label: '月次レポート', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
      { href: '/report-analysis', label: 'レポート分析', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
      { href: '/kpi', label: '自動化設定', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
      { href: '/imports', label: 'インポート (L ステップ)', icon: 'M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12' },
      { href: '/lstep-bridge', label: 'L ステップ Bridge', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1' },
    ],
  },
  {
    label: '設定',
...
function AccountSwitcher() {
  ...
  const filtered = useMemo(() => {
    ...
    return accounts.filter((a) => {
      ...
      const basicId = (a.basicId || '').toLowerCase()
      return name.includes(q) || basicId.includes(q)
    })
  }, [accounts, query])
          ...
          <div className="p-2 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-2 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500"
              />
            </div>
            <p className="text-[10px] text-gray-400 mt-1.5 px-0.5">
              {filtered.length.toLocaleString()} / {accounts.length.toLocaleString()} 件
            </p>
          </div>
          <div className="max-h-80 overflow-y-auto">
            {filtered.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-6">該当するアカウントがありません</p>
            ) : (
...
export default function Sidebar() {
  ...
  const isActive = (href: string) => {
    ...
    // 完全一致 or 末尾が `/` で続く子パスのみ有効化。
    // `/broadcasts` が `/broadcasts/segments` でも光らないようにする。
    if (pathname === href) return true
    if (!pathname.startsWith(href + '/')) return false
    // 子パスを持つ親ナビは、より深い子ナビと同時に光らせない
    if (href === '/broadcasts' && pathname.startsWith('/broadcasts/segments')) return false
    return true
  }
    ...
    <>
      ...
      <div className="px-6 py-5 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-gray-900 leading-tight">L-port</p>
            <p className="text-xs text-gray-400">AI が中の人として動く</p>
          </div>
        </div>
      </div>
        ...
        {menuSections.map((section, si) => (
          <div key={si}>
            ...
            {section.items.filter((item) => {
              ...
              // 顧客画面プレビューは別オリジン (app/staging) を新規タブで開く
              const isClientPreview = item.href === '/client'
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  {...(isClientPreview
                    ? {
                        ...
                        onClick: (e: React.MouseEvent) => {
                          e.preventDefault()
                          window.open(clientPreviewUrl(), '_blank', 'noopener,noreferrer')
                        },
                      }
                    : {})}
      ...
      <div className="border-t border-gray-200">
        ...
        <p className="text-xs text-gray-400">L-port v{process.env.APP_VERSION || '0.0.0'}</p>
        <button
          onClick={() => {
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-gray-900">L-port</p>
        </div>
      </div>
