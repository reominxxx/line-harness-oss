...
// 顧客画面の許可ルート
const CLIENT_ROUTES = [
  '/client', '/client/approvals', '/client/broadcasts',
...
- 質問が曖昧なら聞き返す
- 装飾記号は使わない（**、##、###、--- などの Markdown 記号は禁止。見出しや強調はプレーンな日本語で表現し、箇条書きは「・」を使う）

【レスポンス形式】
回答は JSON で返してください:
{
  "text": "回答文（プレーンテキスト。**・##・--- などの装飾記号は使わない）",
  "actions": [
...
- "none": アクションなし（テキスト回答のみ）`
    : `あなたは L-port（LINE 公式アカウント運用 AI プラットフォーム）の AI アシスタントです。
